...
public class DbContactos extends DbHelper {
    ...
    public Contactos verContato(int id) {
        DbHelper dbHelper = new DbHelper(context);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ...
        cursorContactos = db.rawQuery("SELECT * FROM " + TABLE_CONTACTO + " WHERE id = " + id, null);
        if (cursorContactos.moveToFirst()) {
            contacto = new Contactos();
            contacto.setId(cursorContactos.getInt(0));
            contacto.setNombre(cursorContactos.getString(1));
    ...
    public boolean editarContacto(int id, String nombre, String telefono, String correo) {
        ...
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        try {
            db.execSQL("UPDATE " + TABLE_CONTACTO + " SET nombre = '" + nombre + "', telefono = '" + telefono + "', correo = '" + correo + "' WHERE id = '" + id + "' ");
            correcto = true;
        } catch (Exception e) {
            e.toString();
            correcto = false;
